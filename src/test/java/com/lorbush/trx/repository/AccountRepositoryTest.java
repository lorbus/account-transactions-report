...
public class AccountRepositoryTest {
	...
	public static final String CURRENCY_ID_GBP = "GBP";
	...
	public void before() {
		...
		account1 = new Account(ACCOUNT_IBAN_1, USER, new Currency(CURRENCY_ID_EUR, EUR_CURRENCY, UPDATED_BY),
				new BigDecimal(0), UPDATED_BY);
		account2 = new Account(ACCOUNT_IBAN_2, USER, new Currency(CURRENCY_ID_EUR, EUR_CURRENCY, UPDATED_BY),
				new BigDecimal(0), UPDATED_BY);
	...
	public void whenFindByUserId_thenReturnAccount() {
		List<Account> found = accountRepository.findByUserId(USER);
		assertNotNull(found);
	...
	public void whenFindByUserId_NotFound() {
		List<Account> found = accountRepository.findByUserId("notExistentUserId");
		assertNotNull(found);
	...
	public void whenSave_Success() {
		Account account = new Account(ACCOUNT_IBAN_2, USER, new Currency(CURRENCY_ID_EUR, EUR_CURRENCY, UPDATED_BY),
				new BigDecimal(0), UPDATED_BY);
	...
	public void whenSave_FailWrongCurrency() {
		...
		Account account = new Account(ACCOUNT_IBAN_1, USER, currency, new BigDecimal(0), UPDATED_BY);
		try {
			Account found = accountRepository.save(account);
			fail();
